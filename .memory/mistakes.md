...
### 2026-08-15 — Hugo E25DX: Four Silent Gotchas in Theme Integration

**Symptom:** Hugo site builds cleanly but renders broken (missing navigation, empty sections, no mermaid, literal markdown in HTML).

**Four root causes:**

1. **`[[module.imports]]` without `ignoreConfig = true`:** Theme's `hugo.yaml` defines `theme: E25DX`. Hugo merges that into the project config, then looks for local `themes/E25DX` directory. Without `ignoreConfig = true` in the module import, the merge fails silently (Hugo doesn't error; it just stops using the theme). Fix: Add `ignoreConfig = true` to every `[[module.imports]]` block.

2. **Section pages render empty:** Theme has NO section template. Every section `_index.md` without `layout: single` renders nothing. The home page is the same (data-driven via hero/card-grid yaml, ignores markdown), so both need the explicit layout. Root cause: theme assumes custom layouts exist; they don't. Fix: Add `layout: single` to section/home front matter.

3. **Sidebar requires presence flag:** `data/en/<section>/sidebar.yaml` must EXIST or sidebar partial is omitted, and the entire site loses navigation (stock partial walks content tree; overridden here to use presence flags). Contents are ignored; file existence is the signal. Root cause: poor coupling between partial and data. Fix: Create empty/comment-only yaml files for every section.

4. **Mermaid has no renderer:** Theme ships no third-party JS and its generic codeblock hook only highlights syntax. Mermaid diagrams render as pre text. Fix: Add custom renderer at `layouts/_markup/render-codeblock-mermaid.html`.

**Goldmark extra:** `<div ... markdown>` (md_in_html) does NOT work. Goldmark passes raw HTML through verbatim and never evaluates markdown inside it. Rewrite as HTML — the div structure is preserved for the visual pass.

**Prevention rule:** E25DX requires buy-in on its assumptions (data-driven home, presence-flag sidebars, custom renderers). These are not documented clearly; all four were discovered via trial. Before adopting a Hugo Module theme: (a) test a real section and home page render (not just the theme showcase), (b) search its docs/issues for "empty", "sidebar", "section", "mermaid" to surface known gotchas, (c) keep this list current for future conversions.
