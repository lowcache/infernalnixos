...
**Root cause:** cargo's `cargoSetupPostUnpackHook` examines `cargoVendorDir`; if set, it takes the no-copy branch (uses existing vendor tree at `$cargoVendorDir`). If unset, it falls back to copying vendor sources with `cp -Lr` — which fails identically under proot for the same structural reason. **The two mechanisms are a pair: placing vendor tree in `postUnpack` is inert without setting `cargoVendorDir`.** Only one of them is obviously load-bearing at a glance.
