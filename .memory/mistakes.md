...
### 2026-08-24 — Krita SIGBUS Crashes from Swap Configuration on 4 GB Tmpfs Root

* **Incident (July 2026):** Two unexplained Krita crashes (PIDs 575314, 75335); cores disabled (`Storage: none`), so backtrace unavailable.
* **Root cause (identified 2026-08-24):** Krita's `kritarc:291` default `swaplocation=/tmp` with `maxSwapSize=10240` (10 GB request), but impermanence root `/` is 4 GB tmpfs. When Krita mmap's a swap page, the kernel tries to satisfy it from tmpfs; at 4 GB limit, page fault cannot be satisfied → SIGBUS. Krita does NOT honour `$TMPDIR` env var — it hardcodes `/tmp` regardless of system TMPDIR configuration. Confidence: mechanism is [ESTABLISHED], but specific crash attribution is [SUPPORTED] only (cores missing).
* **Fix applied (2026-08-24):** Changed `kritarc:291 swaplocation=/home/lowcache/Storage/tmp/krita-swap` (NVMe persistent storage, 269 GB free). Crash-free since fix.
* **Prevention rule:** Krita ignores `$TMPDIR`. Override `swaplocation` in `~/.config/kritarc` to point to a persistent large filesystem with >10 GB free. Verify after upgrade (Krita 6.0.3+ may change swaplocation default; always check).
