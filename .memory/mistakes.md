...
* **Root cause identified (systematic debugging):** `waydroid.enable` provides no default persistence. `waydroid init` downloaded ~2.4 GB of system.img + vendor images directly into `/var/lib/waydroid` on the 4 GB impermanence tmpfs root. Additionally, Android Studio SDK already occupied ~1.4 GB at `~/Android` on the same tmpfs (also unpersisted). Total: 3.8 GB of 4 GB root filled, filesystem 100% full. The breakage was not waydroid itself — it was "no space on /" cascading into every other service. Initial misdiagnosis suspected `/persist` (46% used, 142 GB free). It was never involved. The diagnostic: `df -h /` showing tmpfs at 100%, not `/persist` at 100%.
* **Fix applied (2026-08-21):** (1) Added `/var/lib/waydroid` to system persistence in `nixos/hardware-configuration.nix:82`. (2) Symlinked `~/.Android` and `~/.android` to `~/Storage/` via `home/persist.nix:53`. (3) Moved 1.4 GB SDK to Storage (freed 1.4 GB). (4) Staged 2.4 GB image move script awaiting user execution.
* **Prevention Rule:** Before enabling any NixOS service that downloads a runtime image or large blob (waydroid, docker, VM images, SDK managers), add its state directory to `environment.persistence` **in the same commit as the `enable`**. On an impermanence host the default landing zone is tmpfs and the failure presents as unrelated breakage everywhere else, not as an error from the new service. This is the **third recurrence of the pattern** — see `.cache/llmfit` and cache entries in state/persist from 2026-06-16 (Mistakes #10). Diagnostic first: `df -h /` (check tmpfs, not `/persist`). Then `du -xh -d2 /` to measure consumers on tmpfs without following bind-mounts.

### 2026-08-21 — waydroid-nftables Package Variant Shadowed System Service Binary

Symptom: With `virtualisation.waydroid.enable = true` + `waydroid-nftables` installed in user profile, the container service and CLI resolve to different derivations (both version-identical but distinct nix outputs). Root cause: `waydroid-nftables` is a variant build of waydroid with its own binary. Installing it in `/etc/profiles/per-user/…/bin` places it earlier in PATH than `/run/current-system/sw/bin`, so the CLI resolved to the variant closure while the service ran the plain closure. Result: CLI and service binaries were not bytewise-identical, causing potential runtime desync. Prevention rule: When a `*-nftables` / `*-minimal` / `*-full` package variant sits alongside a NixOS service module, check whether it shadows the service binary (`readlink -f $(which <pkg>)` vs `readlink -f /run/current-system/sw/bin/<pkg>`). If the service needs a variant, set `virtualisation.waydroid.package = pkgs.waydroid-nftables` in `configuration.nix` instead of installing into the user profile. Recommendation: This host removed the user-profile install since xtables-nft-multi is already the system default, earning nothing from the variant.
