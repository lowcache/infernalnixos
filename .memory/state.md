...
**Krita 6.0.2.1 + Font Gallery pykrita Plugin (SVG Text Engine Fixed, Plugin Refactored):** Native Krita (Nix build; Flatpak uninstalled 2026-06-22) with Font Gallery plugin (3966-font browser UI, text spinbox). Krita 6.0.1 crashed on SVG text insertion (FreeType glyph rendering bug, upstream fix 2026-05-27). Krita 6.0.2.1 fixed this — SVG `<text>` shapes now render correctly and are fully editable vectors (verified 2026-08-24). **Swap location fix (2026-08-24):** Krita's `kritarc:291` had `swaplocation=/tmp` with `maxSwapSize=10240` (10 GB request), but impermanence tmpfs root is 4 GB → mmap page faults cannot be satisfied → SIGBUS (likely cause of two July crashes, PIDs 575314 & 75335; cores discarded, [SUPPORTED] confidence only). Changed to `swaplocation=~/Storage/tmp/krita-swap` (NVMe, persistent, 269 GB free). Crashes zero since fix. **Font Gallery plugin refactor (2026-08-24):** `_insert_sample` now uses native SVG shapes (`doc.createVectorLayer()` + `addShapesFromSvg()`) instead of QPainter/QImage rasterization. Pure `build_text_svg()` function (testable without Qt/Krita) emits SVG `<text>` with proper XML escaping and multi-line as `<tspan>`. Verified end-to-end via headless test harness (`<scratchpad>/ktest/` — Xvfb + isolated XDG dirs); 4/4 test cases passed (XML metacharacters, multi-line, round-trip), zero crashes. Test harness reusable for future plugin work. Still open: on-canvas GUI text tool (Python API only, not yet GUI-tested). Caveat: `Shape.remove()` in the Python API SEGVs on 6.0.2.1 (separate defect); avoid in plugins. G'MIC plugin patched and bundled (`overrides/gmic-qt-filtersview-nullptr-contextmenu.patch`). Fallback: GIMP 3.2.4 (Flatpak) or 3.0.8 native.
