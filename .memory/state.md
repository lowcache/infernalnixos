...
**Krita 6.0.2.1 + Font Gallery pykrita plugin (2026-08-24 — SVG text engine verified working):** Only the Nix build is in use (`pkgs.krita`, patched G'MIC bundled — see §4). Flatpak uninstalled 2026-06-22 (SIGABRT on launch). **SVG text engine verified working on 6.0.2.1** (isolated harness, Xvfb, real 3966-font set): Vector `<text>` shapes render correctly and are **genuinely editable** (not raster output). The err=84 = `FT_Err_Invalid_Stream_Read` crash that forced the prior raster workaround was **6.0.1-specific**; Krita 6.0.2 (27 May 2026) shipped text crash fixes ("text type checking to prevent potential crashes"). Five test fonts (default, DejaVu Sans, Liberation Sans, JetBrainsMono Nerd Font, Noto Color Emoji) — zero glyph-render errors, zero crashes, verified vector round-trip via `VectorLayer.toSvg()`. Font Gallery (`~/Storage/krita-master/krita/pykrita/font_gallery/`) provides visual 3966-font browser; current implementation rasterizes via `QPainter.drawText` for fallback but can migrate to native `addShapesFromSvg()` path. **Caveat:** `Shape.remove()` in Python API segfaults on generic shapes (control test: crashes on plain rect too) — avoid in pykrita on 6.0.2.1; not text-specific. Krita 6.0.3 (29 July 2026) ships further text fixes (bugs 520939, 517717, 518874, 517361); nixpkgs still on 6.0.2.1. Full root-cause/decision narrative: decisions.md #21 (amended 2026-08-24).
