...
## 21. Native Krita 6.0.2.1 + Font Gallery pykrita Plugin (2026-06-22 — Qt Rasterization with User-Text Input; Amended 2026-07-14 for G'MIC Crash; Amended 2026-08-24 for SVG Text Engine Fix)
...
* **Amendment (2026-08-24 — SVG Text Engine Fix Verified on 6.0.2.1):** **The err=84 crash was 6.0.1-specific and does NOT reproduce on 6.0.2.1.** Isolated harness testing (Xvfb + Qt6, real 3966-font set) confirmed SVG `<text>` insertion via `VectorLayer.addShapesFromSvg()` works without crashes on the current install (6.0.2.1). Five fonts tested (default, DejaVu Sans, Liberation Sans, JetBrainsMono Nerd Font, Noto Color Emoji) — zero glyph-render errors, zero crashes, verified correct PNG exports. **Resulting `KoSvgTextShapeID` shape is genuinely editable vector text** (proven via `VectorLayer.toSvg()` round-trip, not raster). The err=84 = `FT_Err_Invalid_Stream_Read` (verified in freetype-2.14.3 source) was consistent with the corrupt `NoracleNerdFont-Regular.otf` quarantined 2026-06-21, but the crash itself was fixed by the **version bump to 6.0.2** (27 May 2026, shipped "text type checking to prevent potential crashes"). **Consequence:** The raster workaround is **no longer necessary**. Font Gallery can migrate to native `addShapesFromSvg()` path for text insertion. The plugin's *actual* value (visual browser for 3966 fonts) is orthogonal to rendering method. **Caveat:** `Shape.remove()` in Python API segfaults on generic shapes (control test: crashes on plain rect too, not text-specific) — avoid in pykrita on 6.0.2.1. Krita 6.0.3 (29 July 2026) ships further fixes (bugs 520939, 517717, 518874, 517361); nixpkgs still on 6.0.2.1.

* **Why keep this decision:** Font Gallery's font browser UI provides genuine value at 3966 fonts. SVG text engine now works; raster was a necessary workaround that is now obsolete but remains safe fallback. G'MIC patch is local; update nixpkgs when upstream ships a fixed gmic-qt.

* **Honest limitations:** (1) I tested the engine via Python API, not interactive GUI text tool — on-canvas UX unverified. (2) `Shape.remove()` segfaults in Python API — avoid in pykrita. (3) Krita 6.0.3 exists; nixpkgs hasn't updated yet.

* **Installation (current):** Only **Nix** `pkgs.krita` 6.0.2.1 with patched G'MIC plugin (Flatpak uninstalled 2026-06-22). Both SVG text rendering and G'MIC filter tree are now safe from the crashes documented in prior amendments.
