...
let
  ...
  pixelProps = {
    ...
    "ro.bootimage.build.fingerprint" =
      "google/husky/husky:15/AP3A.240905.015.A2/12345678:user/release-keys";
...
in
{
  ...
  environment = {
    ...
    etc = {
      ...
      "libvirt/qemu/android-vm.xml".text =
        let
          vmName = "android-vm";
          cpuCount = 4;
        ...
        ''
          <domain type='kvm'>
            ...
            <qemu:commandline>
              ...
              <qemu:arg value='console=ttyS0 androidboot.hardware=ranchu androidboot.boot_devices=pci.0000:04.00.0 androidboot.verifiedbootstate=green init=/init ro root=/dev/vda1 ro.build.fingerprint=${
                pixelProps."ro.build.fingerprint"
              } androidboot.verifiedbootstate=green'/>
              <qemu:arg value='-initrd'/>
              <qemu:arg value='${systemImg}/ramdisk.img'/>
              <!-- Pixel device properties via qemu command line -->
              ${lib.concatMapStringsSep " " (k: v: "-qemu:arg value=-prop -qemu:arg value=${k}=${v}") (
                builtins.attrNames pixelProps
              )}
            </qemu:commandline>
          </domain>
      ...
      "libvirt/qemu/networks/android-vm-net.xml".text = ''
        <network>
  ...
  systemd = {
    ...
    network.networks."20-android-vm" = {
