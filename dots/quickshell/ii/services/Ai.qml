...
Singleton {
    ...
    property var availableTools: (models[currentModelId] && root.tools[models[currentModelId].api_format]) ? Object.keys(root.tools[models[currentModelId].api_format]) : []
    ...
    onModelsChanged: { const keys = Object.keys(models); if (JSON.stringify(keys) !== JSON.stringify(modelList)) modelList = keys; }
    property var models: (Config.options.policies.ai === 2 ? {} : {
        "gemini-2.5": aiModelComponent.createObject(this, {
            "name": "Gemini 2.5",
        ...
        "gemini-3": aiModelComponent.createObject(this, {
            "name": "Gemini 3",
            "icon": "google-gemini-symbolic",
    ...
    onCurrentModelIdChanged: setModel(currentModelId, false, true)
    property var currentModelId: (root.models[Persistent.states?.ai?.model]) ? Persistent.states.ai.model : "gemini-3"
    ...
    property ApiStrategy currentApiStrategy: (models[currentModelId] && apiStrategies[models[currentModelId].api_format]) ? apiStrategies[models[currentModelId].api_format] : apiStrategies["openai"]
    ...
    function guessModelLogo(model) {
        ...
        if (lower.includes("kimi")) return "spark-symbolic";
        if (lower.includes("glm") || lower.includes("chatglm")) return "spark-symbolic";
        if (lower.includes("minimax")) return "spark-symbolic";
    ...
    function guessModelId(modelId) {
        if (!modelId) return "gemini-3";
        const lower = modelId.toLowerCase();
        if (lower === "gemini-3-flash-preview" || lower === "gemini-3-flash") return "gemini-3";
        if (lower === "gemini-2.5-flash") return "gemini-2.5";
        return modelId;
    }
    ...
    function guessModelName(model) {
        ...
        else if (words.length > 1) words[words.length - 1] = `(${words[words.length - 1]})`;
        
        return words.join(' ');
    }

    function addModel(modelName, data) {
        if (root.models[modelName]) return;
        root.models[modelName] = aiModelComponent.createObject(root, {
            "name": data.name ?? root.guessModelName(data.model),
            "icon": data.icon ?? root.guessModelLogo(data.model),
            "description": data.description ?? "",
            "homepage": data.homepage ?? "",
            "endpoint": data.endpoint,
            "model": data.model,
            "requires_key": data.requires_key ?? false,
            "key_id": data.key_id ?? "",
            "key_get_link": data.key_get_link ?? "",
            "key_get_description": data.key_get_description ?? "",
            ...
            onRead: data => {
                try {
                    ...
                    dataJson.forEach(model => {
                        ...
                        // Prevent overwriting built-in or extra models unless it's intended
                        if (root.models[safeModelName] && !root.models[safeModelName].endpoint.includes("localhost")) return;
                    ...
                    console.log("[AI] Could not fetch Ollama models:", e);
                }
            }
        }
    }
    ...
    function setModel(modelId, feedback, setPersistentState) {
        if (feedback === undefined) feedback = true;
        if (setPersistentState === undefined) setPersistentState = true;
        if (!modelId) modelId = ""
        modelId = root.guessModelId(modelId).toLowerCase()
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: data => {
                ...
                try {
                    const result = requester.currentStrategy.parseResponseLine
                    (data, requester.message);
                    // console.log("[Ai] Parsed response result: ", JSON.stringify(result, null, 2));

                    if (result.thoughtSignature) {
                        requester.message.thoughtSignature = result.thoughtSignature;
                    }

                    if (result.thought) {
                        requester.message.thought = result.thought;
                    }
        ...
        stderr: StdioCollector { onRead: data => { console.log("[AI] Request error: " + data) } }
...
function handleFunctionCall(name, args, message) {
        if (name === "switch_to_search_mode") {
            ...
            const safePrefixes = ["ls", "cat", "grep", "echo", "pwd", "date", "uptime", "find", "head", "tail", "whoami", "neofetch", "fastfetch"];
            const isSafe = safePrefixes.some(prefix => cmd.startsWith(prefix)) && !cmd.includes(">");
            
            if (isSafe) {
                approveCommand(message);
            }
        } else {
